	...
	*/
	...
	function createXHRupload(editor, data) {
		...
		xhr.onload = function() {
			...
			// so let's get out. It can happen also if the upload finish before the element is inserted.
			if (!el)
				return;
	...
	function processFinishedUpload( data ) {
		...
		var	fileUrl = finishedUpload.fileUrl,
			editor = finishedUpload.editor,
			attribute = finishedUpload.attribute;
	...
	function pasteListener(e) {
		...
		for (i = 0; i < items.length; i++) {
			...
			if (!file) {
				if (window.console) {
					console.log('Failed to read file from DataTransferItem');
				}
				continue;
			}
